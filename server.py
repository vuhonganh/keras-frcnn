...
def format_img_channels(img, C):
    ...
    img = img[:, :, (2, 1, 0)]  # this is RGB to BGR
    img = img.astype(np.float32)
    img[:, :, 0] -= C.img_channel_mean[0]  # mean subtraction is in BGR
...
#                   default='resnet50')
# parser.add_option("--write_dir", dest="write_dir", help=
# "write annotated images to directory",
#                   default="./results/")
#
# (options, args) = parser.parse_args()
#
# os.makedirs(options.write_dir, exist_ok=True)
...
############# MODEL COMPILE part : this is common part
...
if __name__ == "__main__":
    ...
    model_rpn, model_classifier_only = load_our_model()
        ...
        if data == EXIT:
            ...
            for key in bboxes:
                ...
                for jk in range(new_boxes.shape[0]):
                    ...
                    rect = patches.Rectangle((real_x1, real_y1), real_x2 - real_x1, real_y2 - real_y1, linewidth=1,
                                             edgecolor='r', facecolor='none')
                    textLabel = '{}: {}%'.format(key, int(100 * new_probs[jk]))
                    all_dets.append((key, 100 * new_probs[jk]))
                    ax.add_patch(rect)
                    ax.text(real_x1, real_y1 - 5, textLabel, fontsize=12, bbox=dict(facecolor='white', alpha=0.5))
                    # ax.text(real_x1, real_y1 - 5, textLabel, color='yellow')

            print('done')
            plt.savefig('cur_res_img.JPEG', frameon=False, bbox_inches='tight')

            img_res = imread('cur_res_img.JPEG')

            # img_res = imresize(img_res, (int(img_res.shape[0] / 2), int(img_res.shape[1] / 2)))
            img_res = imresize(img_res, (264, 352))
            print('write result to cur_res_img.JPEG')
            imsave('cur_res_img.JPEG', img_res)

            res = 'done'

            print('encoding img to string')
            print(img_res.shape)

            msg_from_server = img_res.tostring()
            print('sending to client')
            conn.sendall(msg_from_server + end_from_server.encode())

            # conn.sendall(res.encode())


    print('server saying bye to client')
    conn.sendall(b'bye')
    conn.close()
